...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-baseline justify-between gap-3 border-t border-line py-2">
      ...
      <span className="truncate text-right text-[12px] text-secondary">
        {value}
      </span>
    </div>
  );
...
function NowPlayingCard({
  ...
  return (
    <aside
      className={`flex w-full flex-col gap-4 rounded-[3px] border border-line bg-surface p-4 ${className}`}
    >
      {artwork ? (
        <div
          className="aspect-square w-full rounded-xs bg-cover bg-center min-[1440px]:aspect-auto min-[1440px]:min-h-0 min-[1440px]:flex-1"
          style={{ backgroundImage: `url("${artwork}")` }}
        />
      ) : (
        <div className="grid aspect-square w-full place-items-center rounded-xs border border-line bg-surface-raised min-[1440px]:aspect-auto min-[1440px]:min-h-0 min-[1440px]:flex-1">
      ...
      <div className="shrink-0 space-y-1">
        ...
        <p className="text-[15px] font-semibold leading-tight tracking-[-0.2px] text-primary">
